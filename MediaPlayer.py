...
class MyWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        try:
            ...
            self.framePlayer.setMaximumWidth(456)
    ...
    def fullScreen(self):
        try:
            if self.full and self.mode == 'Video':
                ...
                self.framePlayer.setMaximumWidth(10000)
                self.full = not self.full
        ...
        try:
            if self.mode == 'Audio':
                if event.text() == 'Новый плейлист':
                    ...
                    self.dlg.leUrl.setVisible(False)
                    self.dlg.label_9.setVisible(False)
                    self.dlg.label_7.setText('Добавить новый плейлист')
                    self.dlg.exec()
                    st = self.dlg.data
                    ...
                    for y in resp.items():
                        if self.indActivePlaylist != temp:
                            ...
                            for x in y[1]:
                                if os.path.exists(x[0]['url']):
                                    self.playlist[self.indActivePlaylist - temp45].append(
                                        [x[0]['url'], MP3(x[0]['url']).info])  # Добавляем в плейлист Путь к файлу
                        else:
                            numer = y[0]
                            temp45 += 1
                ...
                if event.text() == 'Новый плейлист':
                    ...
                    for y in resp.items():
                        if self.indActivePlaylist != temp:
                            ...
                            self.playlistMenu.addAction(y[0])
                            for x in y[1]:
                                self.playlist[self.indActivePlaylist - tenp45].append(x[0])
                        else:
                            numer = y[0]
                            temp45 += 1
                        self.indActivePlaylist += 1
                ...
                if event.text() == 'Новый плейлист':
                    ...
                    for y in resp.items():
                        if self.indActivePlaylist != temp:
                            ...
                            self.playlistMenu.addAction(y[0])
                            for x in y[1]:
                                self.playlist[self.indActivePlaylist - temp45].append(x[0])
                        else:
                            numer = y[0]
                            temp45 += 1
                        self.indActivePlaylist += 1
